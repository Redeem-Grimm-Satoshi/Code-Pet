...
class PetSidebarProvider {
    ...
    setSyntaxErrorState(hasError, reason) {
    ...
    pushState(reason) {
        ...
        this.view.webview.postMessage({
            ...
            evolution,
            reason,
            petName: cfg.get('petName', 'Zolix'),
            puppySounds: cfg.get('petPuppySounds', true)
        });
    }
    getHtml() {
...
PetSidebarProvider.viewType = 'codeCompanionPet.view';
function activate(context) {
    ...
    const hasErrorDiagnostics = (uri) => vscode.languages.getDiagnostics(uri).some((d) => d.severity === vscode.DiagnosticSeverity.Error);
    const pushEditorGaze = () => {
        ...
        petProvider.updateGaze(Math.max(-1, Math.min(1, xNorm)), Math.max(-1, Math.min(1, yNorm)));
    };
    ...
    context.subscriptions.push(diagnosticsSub, saveSub, selectionSub, activeEditorSub, rangeSub, testHappyCmd, testAngryCmd);
    pushEditorGaze();
}
function deactivate() { }
